...
class DiagnosticToolWidget(QWidget):
	...
	bad_results = self.bad_results_bool
	    ...
	    (result1, outlier_limit_neg, outlier_limit_pos) = diagnostic.check_for_spikes(spikes, actuator_name, debug_info)
            ...
            if bad_results:
	        if result: 
                    param = (filename,velocity,spikes,acceleration,outlier_limit_neg,outlier_limit_pos,supply_voltage,measured_motor_voltage,executed_current,measured_current)
                    self.plots.append(filename +'_1')
		    self.plots.append(filename +'_2')
