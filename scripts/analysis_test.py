#!/usr/bin/env python
import roslib; roslib.load_manifest('pr2_diagnostic_tool')
...
class Diagnostic():
  ...
  def check_for_spikes(self, spikes, actuator_name):
    outlier_limit_neg = 0
    outlier_limit_pos = 0
  ...
  def check_for_unplugged(self, velocity, measured_motor_voltage, actuator_name):
  ...
  def check_for_open(self, velocity, measured_motor_voltage, actuator_name):
...
if __name__ == '__main__':
  ...
  for filename in filelist:
    ...
    (result1, outlier_limit_neg, outlier_limit_pos) = diagnostic.check_for_spikes(spikes, actuator_name) 
    result2 = diagnostic.check_for_unplugged(velocity, measured_motor_voltage, actuator_name)
    result3 = diagnostic.check_for_open(velocity, measured_motor_voltage, actuator_name)
